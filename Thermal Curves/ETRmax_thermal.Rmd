...
have a look
```{R}
ggplot(rlc_metrics, aes(x = as.numeric(Well), y = Pmax, col = SampleName)) +
    geom_line(alpha = 0.6) +
    geom_point() +
...
for (sample_name in samples) {
    ...
    # sample_data <- filter(data, SampleName == sample_name, Well != 6) # remove last temperature due to extreme spike
    ...
    # print(plot)
    ggsave(paste0("Thermal Curves/ETRmax/plots/", sample_name, "-6.png"), plot)
}

print(failures)

write.csv(outputs, "Thermal Curves/ETRmax/6 raw output.csv")
```
