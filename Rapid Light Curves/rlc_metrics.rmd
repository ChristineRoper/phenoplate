...
for (sample in sampleNames) {
    ...
    for (well in 1:6) {
        ...
        output <- rbind(output, data.frame(
            ...
            Ek = Ek,
            Iopt = Iopt,
            qy_ek = Eilers_QY_PI(QY_max, Iopt, Ek, Ek),
            AIC = AIC(fit),
        ...
        sampleData[sampleData$Well == well, "QY_fit"] <- Eilers_QY_PI(QY_max, Iopt, Ek, pars)


        plot <- plot +
            geom_hline(yintercept = QY_max, color = "grey", linetype = "dashed", alpha = 0.2) +
            geom_segment(data = data.frame(QY_max = QY_max, Ek = Ek), mapping = aes(x = 0, y = QY_max, xend = Ek, yend = 0), color = "grey", linetype = "dashed", alpha = 0.2) +
            geom_vline(xintercept = Ek, color = "grey", linetype = "dashed", alpha = 0.2)
