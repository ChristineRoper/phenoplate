...
stdErrs = aggregate(ETRmax_data$ip, by=list(ETRmax_data$Group), stdErr)
means = aggregate(ETRmax_data$ip, by=list(ETRmax_data$Group), mean)
...
#log logistic plots averaged by group
```{r}
input <- "2. Rapid Light Curves/outputs/rlc_etr_metrics_eilers.csv"
plots <- "Parameter stats analysis/Averaged ED50 Plots/"
...
mean_by_group <- aggregate(data, ETR_max ~ Temperature + Group, mean)


DRCpam <- drm(ETR_max ~ Temperature,
    data = mean_by_group, curveid = Group,
    fct = LL.3(names = c("hill", "max", "ed50"))
)
compParm(DRCpam, "ed50")
compParm(DRCpam, "ed50", "-")
plot(DRCpam, col = 1:15, lty = 1, lwd = 2, pch = 1,legend = FALSE)
